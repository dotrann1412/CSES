...
void solve(){
	...
	for(int i = 0; i < n; ++i) {
		for(int j = 0; j < m; ++j) {
			if(g[i][j] == 'A') start = pii(i, j);
			if(g[i][j] == 'B') end = pii(i, j), g[i][j] = '.';
		}
	}
		...
		while(!que.empty()) {
			...
			for(int i = 0; i < 4; ++i) {
				if(isvalid_idx(cur.fi + dx[i], cur.se + dy[i]) 
					...
					par[cur.fi + dx[i]][cur.se + dy[i]] = cur;
					
					g[cur.fi + dx[i]][cur.se + dy[i]] = '#';
					que.push({cur.fi + dx[i], cur.se + dy[i]});

					if(pii(cur.fi + dx[i], cur.se + dy[i]) == end) return;
				}
			}
		}
	};
	...
	if(par[end.fi][end.se] == pii(-1, -1)) cout << "NO" << '\n';
	else {
		stack<char> ans;
		...
		while(pii(x, y) != start) {
			...
			if(y != par[x][y].se) {
				if(y < par[x][y].se) ans.push('L');
				else ans.push('R');
				y = par[x][y].se;
			} else {
				if(x < par[x][y].fi) ans.push('U');
				else ans.push('D');
				x = par[x][y].fi;
			}
		}
